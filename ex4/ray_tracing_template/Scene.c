...
void createScene(Scene* scene) {
    ...
    // Sphere 4:
    (scene->_spheres[3])._radius = 5.f;

    (scene->_spheres[3])._center._x = 0.0f;
    (scene->_spheres[3])._center._y = -5.1f;
    (scene->_spheres[3])._center._z = -3.5f;

    (scene->_spheres[3])._color._red = 0.f;
    (scene->_spheres[3])._color._green = 0.5f;
    (scene->_spheres[3])._color._blue = 1.f;
    ...
    // Cube
    scene->_number_cubes = 1;

    scene->_cubes[0]._center = (Vector3){-2.0f, -2.0f, -4.0f};
}
